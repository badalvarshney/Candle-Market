...
export default function CheckoutModal({
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      setOrderId(res.orderId || res._id);
      setStep(3);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="relative w-full max-w-2xl bg-white border border-[#E8E3DA] p-4 sm:p-8 max-h-[92vh] overflow-y-auto shadow-2xl">
        ...
        {step !== 3 && (
          <button
            onClick={onClose}
            className="absolute top-3 right-3 sm:top-4 sm:right-4 p-2 text-stone-400 hover:text-stone-800 transition-colors cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        )}
        ...
        {step === 1 && (
          <form onSubmit={handleSubmitShipping} className="space-y-4 text-xs">
            ...
            <div className="pt-2 border-t border-[#E8E3DA]">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="btn-outline-kimirica text-[10px] py-2 px-4 whitespace-nowrap"
                >
                  APPLY
                </button>
              </div>
            ...
            <div className="bg-[#F9F5F0] p-4 border border-[#E8E3DA] text-[#111827] flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3 mt-3">
              ...
              <button type="submit" className="btn-kimirica text-xs py-3 px-6 justify-center cursor-pointer">
        ...
        {step === 2 && (
          <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between pt-4 border-t border-[#E8E3DA] gap-3">
              <button
                onClick={() => setStep(1)}
                className="text-xs font-semibold text-[#6B7280] hover:text-[#111827] uppercase tracking-wider underline text-center sm:text-left cursor-pointer"
              >
                ← EDIT ADDRESS
              </button>
              ...
              <button
                ...
                className="btn-kimirica text-xs py-3 px-8 shadow-md justify-center cursor-pointer disabled:opacity-50"
              >
                <span>{placingOrder ? 'PROCESSING ORDER...' : `PLACE ORDER • ₹${currentFinalTotal}`}</span>
              </button>
            </div>
          </div>
        )}
          ...
          <div className="text-center py-4 space-y-6">
            ...
            <div>
              ...
              <p className="text-xs text-[#6B7280] uppercase tracking-wider mt-1 max-w-md mx-auto font-light">
                Your order has been recorded into our database. Confirmation receipt sent to <strong>{formData.email}</strong>.
              </p>
            </div>
            ...
            <button onClick={onClose} className="btn-kimirica text-xs py-3 px-8 mx-auto justify-center cursor-pointer">
