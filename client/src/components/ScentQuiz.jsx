...
export default function ScentQuiz({ onQuickView, onAddToCart, candlesList = [] }) {
  ...
  const questions = [
    ...
    },
    {
      ...
      options: [
        ...
        { label: "Silent Organic Egyptian Cotton Wick", val: "cotton", icon: Shield },
  ...
  const handleSelectOption = (questionId, option) => {
    ...
    if (currentStep < questions.length - 1) {
      ...
      const match = candlesList.find(c => c.scentFamily === preferredCategory) || candlesList[0];
          ...
          <div className="flex items-center justify-between mb-8 pb-4 border-b border-[#E8E3DA]">
            ...
            <div className="flex items-center gap-1">
              {questions.map((_, idx) => (
                <div
                  key={idx}
                  className={`h-1.5 transition-all ${idx === currentStep
                      ? 'w-8 bg-[#C59B27]'
                      : idx < currentStep
                        ? 'w-3 bg-[#1B3B32]'
                        : 'w-3 bg-stone-200'
